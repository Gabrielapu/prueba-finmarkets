...
<template>
  ...
  <div class="uk-flex uk-flex-between">
    <span>1 MES</span>
    <b v-if="selectedResume" :class="getVariationClass(resumeSelectedInstrument.price.pct30D)">
      {{ resumeSelectedInstrument.price.pct30D.toFixed(2) }}%
    </b>
  </div>
  <div class="uk-flex uk-flex-between">
    <span>1 AÑO</span>
    <b v-if="selectedResume" :class="getVariationClass(resumeSelectedInstrument.price.pctRelW52)">
      {{ resumeSelectedInstrument.price.pctRelW52.toFixed(2) }}%
    </b>
  </div>
  <div class="uk-flex uk-flex-between">
    <span>AÑO A LA FECHA</span>
    <b v-if="selectedResume" :class="getVariationClass(resumeSelectedInstrument.price.pctRelCY)">
